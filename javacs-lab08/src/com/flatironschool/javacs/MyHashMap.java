...
public class MyHashMap<K, V> extends MyBetterMap<K, V> implements Map<K, V> {
	...
	protected void rehash() {
		List<MyLinearMap<K,V>> savedMaps = maps; //save old maps
		makeMaps(maps.size() * 2); //double the number of maps
		//put old maps into new map
		for(MyLinearMap<K,V> map: savedMaps) {
